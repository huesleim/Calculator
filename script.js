...
numbers.forEach((button) => {
    button.addEventListener("click", () => {
        ...
        if (button.textContent.trim() === "." && currentNumber.includes("."))
            return;
        if (currentNumber === "0" && button.textContent.trim() === "0") return;
        if (button.textContent.trim() === "." && currentNumber === "") {
            currentNumber = "0.";
            refreshDisplay();
            return;
        }
        if (currentNumber === "0" && button.textContent.trim() !== ".") {
